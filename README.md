...
현재 구현된 범위는 정적 스키마, strong ID, 기본 Memorial 도메인 스키마, 컴파일타임 검증, 런타임 ID·시간·provenance 타입과 typed node SoA 프로토타입이다. 완전한 delta 저장소, 이벤트 로그, snapshot, 세계선 및 쿼리 실행은 아직 구현되지 않았다.
