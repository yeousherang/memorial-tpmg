...
현재 구현된 범위는 정적 스키마, strong ID, 기본 Memorial 도메인 스키마, 컴파일타임 검증, 런타임 ID·시간·provenance 타입, append-only typed node delta, relation별 양방향 adjacency, 불변 generation snapshot, sequence 기반 이벤트 로그, checkpoint/replay와 canonical state/log hash다. 세계선 분기 및 쿼리 실행은 아직 구현되지 않았다.
